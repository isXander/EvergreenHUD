...
class ColorSettingComponent(val component: SettingComponent, val setting: ColorSetting) : ColorComponent({ setting.get() }) {
    ...
    inner class ColorPopout : UIBlock(EvergreenPalette.Greyscale.Dark2.awt) {
        ...
        private val hueColorList: List<Color> = (0..50).map { i -> Color(java.awt.Color.HSBtoRGB(i / 50f, 1f, 0.7f), false) }
        ...
        private fun updateColor() {
            setting.set(Color(java.awt.Color.HSBtoRGB(currentHue, currentSaturation, currentBrightness), false, setting.get().chroma).withAlpha(setting.get().alpha))
        }

        private fun getHueRGB(): Color {
            return Color(java.awt.Color.HSBtoRGB(currentHue, 1f, 1f), false)
        }
        ...
        private fun UIComponent.drawHueLine(matrices: UMatrixStack) {
            ...
            matrices.tessellate(UGraphics.DrawMode.QUADS, DefaultVertexFormats.POSITION_COLOR) {
                var first = true
