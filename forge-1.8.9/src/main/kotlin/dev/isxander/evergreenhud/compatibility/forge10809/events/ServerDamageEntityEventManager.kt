...
package dev.isxander.evergreenhud.compatibility.forge10809.events

import dev.isxander.evergreenhud.EvergreenHUD
import dev.isxander.evergreenhud.compatibility.forge10809.impl.EntityImpl
    ...
    private fun onPacket(packet: Packet<*>) {
        if (packet is S19PacketEntityStatus) {
            ...
            if (attacker != null && targetId == target.entityId) {
                EvergreenHUD.EVENT_BUS.post(ServerDamageEntity(EntityImpl(attacker!!), EntityImpl(target)))
                attacker = null
                targetId = -1
            }
        }
