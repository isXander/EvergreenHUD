...
class Position2D private constructor(
    ...
    var rawX: Float
        get() = (origin.x + scaledX) * (resolution.scaledWidth / 2)
        set(x) {
            origin = calculateOrigin(x, rawY)
            scaledX = calculateScaledX(x, origin)
        }
    var rawY: Float
        get() = (origin.y + scaledY) * (resolution.scaledHeight / 2)
        set(y) {
    ...
    companion object {
        fun rawPositioning(x: Float, y: Float, scale: Float = 1f, origin: Origin = calculateOrigin(x / resolution.scaledWidth, y / resolution.scaledHeight)): Position2D =
        ...
        fun calculateScaledX(rawX: Float, origin: Origin): Float =
            rawX * (resolution.scaledWidth / 2) - origin.x
        fun calculateScaledY(rawY: Float, origin: Origin): Float =
            rawY * (resolution.scaledHeight / 2) - origin.y
    }
